...
const getTaskStatusController = async (req, res) => {
    try {
        ...
        if (!taskStatuses || taskStatuses.length === 0) {
            return res.status(404).json({ status: 'false', message: 'No task statuses found' });
        }
        ...
        res.status(500).json({ status: 'false', message: ['Error fetching task statuses', error.message] });
    }
}
...
const postTaskStatusController = async (req, res) => {
    try {
        ...
        const newTaskStatus = new taskStatusModel({ TaskStatus, shortForm, colorCode });
        await newTaskStatus.save();
        
        res.status(201).json({ status: 'true', data: newTaskStatus });
    } catch (error) {
        res.status(500).json({ status: 'false', message: ['Error creating task status', error.message] });
    }
}
...
const putTaskStatusController = async (req, res) => {
    try {
        ...
        if (!updatedTaskStatus) {
            return res.status(404).json({ status: 'false', message: 'Task status not found' });
        }

        res.status(200).json({ status: 'true', data: updatedTaskStatus });
    } catch (error) {
        res.status(500).json({ status: 'false', message: ['Error updating task status', error.message] });
    }
}
...
const deleteTaskStatusController = async (req, res) => {
    try {
        ...
        res.status(200).json({ status: 'true', message: 'Task status deleted successfully' });
    } catch (error) {
        res.status(500).json({ status: 'false', message: ['Error deleting task status', error.message] });
    }
}
