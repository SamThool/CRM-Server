...
const getNetworkController = async (req, res) => {
  try {
    ...
    if (!networks || networks.length === 0) {
      return res.status(404).json({ status: 'false', message: "No networks found" });
    } 
...
const createNetworkController = async (req, res) => {
  try {
    ...
    const exists = await NetworkModel.findOne({ networkName });
    if (exists) return res.status(400).json({ status: 'false', message: "Network Name already exists" });
...
const updateNetworkController = async (req, res) => {
  try {
    ...
    const updated = await NetworkModel.findByIdAndUpdate(id, { networkName }, { new: true });
    if (!updated) return res.status(404).json({ status: 'false', message: "Network not found" });
...
const deleteNetworkController = async (req, res) => {
  try {
    ...
    if (!deleted) return res.status(404).json({ status: 'false', message: "Network not found" });
